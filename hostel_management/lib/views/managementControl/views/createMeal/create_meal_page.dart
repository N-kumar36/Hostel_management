...
class _MealManagementPageState extends State<MealManagementPage> {
  ...
  Future<void> _initData() async {
    //  1. Fetch current cycle bounds first before compiling records
    await _loadCurrentCycleTimeframe();

    // 2. Fetch meals matching the active bounds constraints
    await _fetchMeals();
    if (!mounted) return;
  ...
  //  Syncs active cycle date boundaries down to memory
  Future<void> _loadCurrentCycleTimeframe() async {
    try {
      final res = await api.getMealCycleDateBounds();
      if (res['success'] == true && res['cycles'] != null && mounted) {
        final List<dynamic> cycleList = res['cycles'];

        // Find the map where isCurrentActive equals true
        final currentActiveCycle = cycleList.firstWhere(
        ...
        if (currentActiveCycle != null) {
          setState(() {
            activeCycleStartDate = currentActiveCycle['startDateStr']
                ?.toString();
            activeCycleEndDate = currentActiveCycle['endDateStr']?.toString();
          });
          debugPrint(
            "Active Engine Bounds Intercepted: $activeCycleStartDate to $activeCycleEndDate",
          );
        } else {
          debugPrint("No active cycle indicator found in the system registry.");
        }
      }
    } catch (e) {
      debugPrint(
        "Could not intercept cycle date limits fallback triggered: $e",
      );
    }
  }
  ...
  void _loadMealForEdit(Map<String, dynamic> meal) {
    setState(() {
      try {
        ...
        if (morningRawTime.toString().contains("T")) {
          final timePart = morningRawTime.toString().split("T")[1];
          final parts = timePart.split(":");
          morningLock = TimeOfDay(
            hour: int.parse(parts[0]),
            minute: int.parse(parts[1]),
          );
        } else {
          morningLock = const TimeOfDay(hour: 7, minute: 0);
        }

        if (nightRawTime.toString().contains("T")) {
          final timePart = nightRawTime.toString().split("T")[1];
          final parts = timePart.split(":");
          nightLock = TimeOfDay(
            hour: int.parse(parts[0]),
            minute: int.parse(parts[1]),
          ...
          nightLock = const TimeOfDay(hour: 17, minute: 0);
        }
      } catch (e) {
        debugPrint("Error loading structural targets inside form elements: $e");
      }
    });
  }
  ...
  Future<void> _submitForm() async {
    ...
    final String morningTimeStr =
        '${morningLock.hour.toString().padLeft(2, '0')}:${morningLock.minute.toString().padLeft(2, '0')}:00';
    final String nightTimeStr =
        '${nightLock.hour.toString().padLeft(2, '0')}:${nightLock.minute.toString().padLeft(2, '0')}:00';

    final payload = {
      "date": DateFormat('dd/MM/yyyy').format(selectedDate),
      "morning": {
        "manu": selectedMorningMenu,
        "lockTime": morningTimeStr,
        "isCancelled": false,
      },
      "night": {
        "manu": selectedNightMenu,
        "lockTime": nightTimeStr,
        "isCancelled": false,
      },
    };
    ...
    try {
      ...
      if (mounted) {
        ...
        // 🌟 FIX: Refresh cycle bounds from server immediately so the new meal is not hidden by out-of-date parameters
        await _loadCurrentCycleTimeframe();
        await _fetchMeals();
      }
    } catch (e) {
  ...
  Future<void> _pickDate() async {
    ...
    // 🛡️ Safety Guard: Ensure initialDate stays strictly bounded inside firstDate and lastDate boundaries
    DateTime initialPickerDate = selectedDate;
    if (initialPickerDate.isBefore(today.subtract(const Duration(days: 7)))) {
      initialPickerDate = today;
    }

    // Set a much safer upper bound constraint context (e.g., end of the current active cycle year)
    final DateTime maxAllowedDatePickerDate = today.add(
      const Duration(days: 90),
    );
    if (initialPickerDate.isAfter(maxAllowedDatePickerDate)) {
      initialPickerDate = maxAllowedDatePickerDate;
    }
    ...
    final DateTime? picked = await showDatePicker(
      ...
      lastDate:
          maxAllowedDatePickerDate, // Dynamic safe layout boundary limit extension
    );
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) {
          final int daysCount =
              dialogEndDate.difference(dialogStartDate).inDays + 1;
                      ...
                      if (picked != null) {
                        setDialogState(() {
                          ...
                          if (dialogEndDate.isBefore(dialogStartDate)) {
                            dialogEndDate = dialogStartDate.add(
  ...
  Future<void> _triggerAutoGenerate(
    ...
    try {
      ...
      if (res['success'] == true && mounted) {
        ...
        // 🌟 FIX: Refresh cycle bounds from server immediately after bulk generation blocks execute
        await _loadCurrentCycleTimeframe();
        await _fetchMeals();
      } else if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        actions: [
          // 🌟 UPDATED SYNC ACTION: Refetches both dates and array definitions securely
          IconButton(
            onPressed: () async {
              await _loadCurrentCycleTimeframe();
              await _fetchMeals();
      ...
      body: Column(
        children: [
          _buildFormSection(),
          if (activeCycleStartDate != null && activeCycleEndDate != null)
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
              child: Align(
                alignment: Alignment.centerRight,
                child: Text(
                  "Active Cycle: $activeCycleStartDate to $activeCycleEndDate",
                  style: TextStyle(
                    fontSize: 11,
                    fontWeight: FontWeight.bold,
  ...
  Widget _buildMealsList() {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final String morningNum =
            meal['morning']?['mealsNum']?.toString() ?? "N/A";
        ...
        if (!isEdit) ...[
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              onPressed: isLoading || isRoutineLoading
                  ? null
  ...
  String _formatLockTime(String isoString) {
    try {
      if (isoString.isEmpty || !isoString.contains("T")) return "N/A";
      final timePart = isoString.split("T")[1].substring(0, 5);
      final parts = timePart.split(":");
      final hour = int.parse(parts[0]);
      final minute = int.parse(parts[1]);

      final parsedDateTime = DateTime(2026, 1, 1, hour, minute);
  ...
  Widget _buildCancelTile(
    ...
    return ListTile(
      ...
      leading: Icon(
        ...
        color: isCancelled
            ? Colors.grey
            : (isMorning ? Colors.orange : Colors.indigo.shade400),
        size: 18,
      ),
      title: Row(
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: isCancelled
                  ...
                  : (isMorning ? Colors.orange.shade50 : Colors.indigo.shade50),
