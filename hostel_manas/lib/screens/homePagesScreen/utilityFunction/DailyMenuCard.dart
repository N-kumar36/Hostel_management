...
class DailyMenuCardState extends State<DailyMenuCard> {
  ...
  Future<void> _handleVoteAction(String slot, bool currentlyVoted, bool isLocked, bool isCancelled) async {
  ...
  Widget build(BuildContext context) {
    ...
    if (todayMeal == null) return const Center(child: Text("No menu set for today."));
    ...
    return Container(
      ...
      child: Column(
        children: [
          _menuRow("Morning", todayMeal!['morning']['manu'], morningStates, Colors.orange),
          const Divider(height: 24),
          _menuRow("Night", todayMeal!['night']['manu'], nightStates, Colors.indigo),
        ],
      ),
    );
  }

  ({bool isLocked, bool isCancelled, bool isServed, bool voted, String formattedTime})
  _getMealStates(Map<String, dynamic> meal, String slot) {
    final lockTime = _parseLockTime(DateFormat('dd/MM/yyyy').format(DateTime.now()), meal['lockTime'] ?? "");
    ...
    final isLocked = (meal['isLocked'] == true) || DateTime.now().isAfter(lockTime) || isServed;
    final isCancelled = meal['isCancelled'] == true;
    final formattedTime = DateFormat('hh:mm a').format(lockTime);

    return (isLocked: isLocked, isCancelled: isCancelled, isServed: isServed, voted: voted, formattedTime: formattedTime);
  }
    ...
    return Row(
      children: [
        Container(
          ...
          child: Icon(label == "Morning" ? Icons.wb_sunny : Icons.nightlight_round, color: color, size: 20),
        ),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Text(s.isCancelled ? "CANCELLED" : dish.toUpperCase(), style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: s.isCancelled ? Colors.red : Colors.black)),
                  if (s.isServed) ...[
              ...
              Text(s.isServed ? "Meal consumed" : (s.isLocked ? "Voting Closed" : "Ends at ${s.formattedTime}"), style: TextStyle(fontSize: 11, color: (s.isLocked || s.isServed) ? Colors.blueGrey : Colors.grey)),
            ],
          ),
        ),
        ElevatedButton(
          onPressed: (s.isLocked || s.isCancelled || s.isServed || isActionLoading) ? null : () => _handleVoteAction(label.toLowerCase(), s.voted, s.isLocked, s.isCancelled),
          style: ElevatedButton.styleFrom(backgroundColor: btnColor, foregroundColor: Colors.white, disabledBackgroundColor: btnColor.withOpacity(0.6), elevation: 0, minimumSize: const Size(80, 35), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
          child: isActionLoading
              ? const SizedBox(width: 15, height: 15, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
              : Text(buttonText, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold)),
        ),
      ],
    );
  }
