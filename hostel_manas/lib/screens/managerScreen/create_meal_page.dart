...
class _MealManagementPageState extends State<MealManagementPage> {
  ...
  // ✅ BUG FIX: If the next logical date is in the past, skip to TODAY
  void _autoSelectNextDate() {
    ...
    try {
      ...
      // Check if the calculated next date is in the past
      final now = DateTime.now();
      final today = DateTime(now.year, now.month, now.day);
      if (nextDate.isBefore(today)) {
        nextDate = today; // Reset to today if the mess was closed
      }
  ...
  // ✅ UPDATED: Auto Generate Dialog shows exact Days count
  void _showAutoGenerateDialog() {
    ...
    // Default lock times for the generated meals
  ...
  Future<void> _triggerAutoGenerate(
    ...
    setState(() => isLoading = true);
    try {
      // Format times as "HH:mm" (24-hour format) for the backend to parse easily
